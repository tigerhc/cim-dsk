...
public class FabRoleController extends BaseBeanController<FabRole> {
    @Autowired
    private IFabRoleService IotRoleService;
    ...
    public void list( HttpServletRequest request) throws IOException {
        //加入条件
        EntityWrapper<FabRole> entityWrapper = new EntityWrapper<>(FabRole.class);
        ...
        FastJsonUtils.print(pageBean, FabRole.class,"id,name,code,isSys,usable");
    }
    ...
    public List<FabRole> usableLst() throws IOException {
        EntityWrapper<FabRole> entityWrapper = new EntityWrapper<FabRole>(FabRole.class);
        entityWrapper.orderBy("createDate",false);
        List<FabRole> usableLst = IotRoleService.selectList(entityWrapper);
        return usableLst;
    }
    ...
    public Response add(FabRole entity, BindingResult result) {
        // 验证错误
    ...
    public Response update(FabRole entity, BindingResult result) {
        // 验证错误
        this.checkError(entity,result);
        entity.setDelFlag("0");
