...
public class MesLotTrackController extends BaseCRUDController<MesLotTrack> {
    ...
    @RequestMapping(value = "/findSecondSinteringParam/{eqpId}", method = {RequestMethod.GET, RequestMethod.POST})
    public String findSecondSinteringParam(Model model, @PathVariable String eqpId, @RequestParam String opId,
                                           HttpServletRequest request, HttpServletResponse response) {
        log.info("findSecondSinteringParam :  {}, {}", opId, eqpId);
        String eventDesc = "{\"eqpId\":\"" + eqpId + "\",\"opId\":\"" + opId + "\"}";//日志记录参数
        try {
            fabLogService.info(eqpId, "Param11", "MesLotTrackController.findSecondSinteringParam", eventDesc, "", "wangdong");//日志记录参数
            //String eqpId ="SIM-DM1";
            if("".equals(opId) || opId == null){
                return "opId Cannot be empty";
            }
            MesResult result = mesLotTrackService.findPrinterParam(eqpId, opId);
            JSONObject jo = JSONObject.fromObject(result);//日志记录结果
            fabLogService.info(eqpId, "Result11", "MesLotTrackController.findSecondSinteringParam", jo.toString(), eqpId, "wangdong");//日志记录
            if ("Y".equals(result.getFlag())) {
                return result.getContent().toString();
            } else {
                return result.getMsg();
            ...
            fabLogService.info(eqpId, "Error11", "MesLotTrackController.findSecondSinteringParam", "有异常", eqpId, "wangdong");//日志记录
            return e.getMessage();
        }
    }*/
