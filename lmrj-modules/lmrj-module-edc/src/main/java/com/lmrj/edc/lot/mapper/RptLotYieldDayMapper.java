...
public interface RptLotYieldDayMapper extends BaseMapper<RptLotYieldDay> {
    ...
    @Select("select lot_yield,lot_yield_eqp,eqp_id from rpt_lot_yield_day where station_code=#{stationId} and period_date=#{date}")
    List<Map<String,Object>> findEqp( @Param("stationId") String stationId,@Param("date") String date);
}
