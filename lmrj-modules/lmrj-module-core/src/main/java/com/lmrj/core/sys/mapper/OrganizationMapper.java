...
public interface OrganizationMapper extends BaseTreeMapper<Organization> {
	...
	@Select("select distinct STEP_CODE name , max(SORT_NO) sortNo  from fab_equipment WHERE line_no =#{userId} AND wip_sub_line_no = #{wipSubLineNo}  AND step_yield_flag='1' group by STEP_CODE  order by max(SORT_NO) ")
	List<Organization> findYieldStep(String userId,String wipSubLineNo);
