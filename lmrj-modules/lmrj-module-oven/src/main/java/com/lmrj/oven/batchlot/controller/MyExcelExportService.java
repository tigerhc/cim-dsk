...
public class MyExcelExportService extends BaseExportService {
    ...
    public void createSheet(Workbook workbook, ExportParams entity, Class<?> pojoClass, Collection<?> dataSet, int titleRows) {
        ...
        try {
            ...
            getAllExcelField(entity.getExclusions(), targetId, fileds, excelParams, pojoClass,
                    null, null);
            //获取所有参数后,后面的逻辑判断就一致了
            createSheetForMap(workbook, entity, excelParams, dataSet, titleRows);
        } catch (Exception e) {
    ...
    public void createSheetForMap(Workbook workbook, ExportParams entity, List<ExcelExportEntity> entityList, Collection<?> dataSet, int titleRows) {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Excel version is {}",
                    entity.getType().equals(ExcelType.HSSF) ? "03" : "07");
        }
        ...
        insertDataToSheet(workbook, entity, entityList, dataSet, sheet, titleRows);
    }
...
//            } else {
//                sheet.createFreezePane(0, titleRows);
//            }
            sheet.createFreezePane(0, titleRows); //TODO
            ...
            if (dataSet.size() > 0) {
                createSheetForMap(workbook, entity, entityList, dataSet, titleRows);
            } else {
                // 创建合计信息
