...
public interface OvnBatchLotMapper extends BaseMapper<OvnBatchLot> {
    ...
    int newfindCountBytime(@Param("eqpId") String eqpId,@Param("beginTime") String beginTime, @Param("endTime") String endTime);

    List<Map> findDetailBytime(@Param("eqpId") String eqpId,@Param("beginTime") String beginTime, @Param("endTime") String endTime);

    List<Map> newfindDetailBytime(@Param("eqpId") String eqpId,@Param("beginTime") String beginTime, @Param("endTime") String endTime);
    ...
    @Select("select *  from ovn_batch_lot where eqp_id = #{eqpId} and ( (end_time > #{beginTime} AND end_time < #{endTime}) OR (start_time > #{beginTime} AND start_time < #{endTime}) OR (start_time < #{beginTime} AND end_time > #{endTime}))  ORDER BY start_time")
    List<OvnBatchLot> findDataByEqpId(@Param("beginTime")String beginTime, @Param("endTime")String endTime,@Param("eqpId") String eqpId);
