...
public class Oauth2Controller {
    ...
    public Object authorize(Model model, HttpServletRequest request) throws URISyntaxException, OAuthSystemException {
        try {
            ...
            if (!StringUtil.isEmpty(loginSource)&&loginSource.equals("client")) {
                //得到到客户端重定向地址
    ...
    private boolean login(Subject subject,String username,String password,String captcha,HttpServletRequest request) {
        ...
        try {
            ...
            redisTemplate.opsForValue().set(token.toString(),user,10000000L);
            ...
            request.setAttribute("error", "redis连接失败");
            LoginLogUtils.recordFailLoginLog(username,"redis连接失败");
            return false;
        }catch (Exception e) {
